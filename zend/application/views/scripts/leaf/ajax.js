...
$(window).bind("popstate", function(e){
    ...
    function validate(json)
    {
                ...
                {
                    window.location.href=json.userid;
                }
                else if(json.status=='false')
                    {
                    document.getElementById("userid").value='';
                    document.getElementById("password").value='';
                    window.location.href='loginattempt?error=1og_fail';
                    }
                else if(json.redirect!='false'){
                	window.location=json.redirect;
                }
                    else
                    window.location='index/secondstep';
    ...
    function sendmsguser(userid,element){
    	...
    	$.ajax({
    		url:'http://www.freniz.com/sendmessages',
    		cache:false,
    		data:parameters,
    		type:'post',
    		dataType:"json",
      ...
      function createb()
       ...
       var text=escape(document.blogmessage.blg.value);
          ...
          if(text==''){alert("please fill the information");}else{
        		...
        		alert('http://www.freniz.com/addblog?'+parameters);
            	$.ajax({
            		url:'http://www.freniz.com/addblog',
            		cache:false,
            		data:parameters,
            		type:'post',
            		dataType:"json",
      ...
      function getblogtemp(userid){
       	 $.ajax({
       		    url:'http://www.freniz.com/blog/'+userid+'?format=xml&from='+postsfrom.blogs,
       		    cache:false,
       		    dataType:"xml",
      ...
      function deleteblog(blogid){
    	  $.ajax({
  		    url:'http://www.freniz.com/deleteblog?blogid='+blogid,
  		    cache:false,
  		    dataType:"json",
  		    success:function(json){
      ...
      {
    	  $.ajax({
    		    url:'http://www.freniz.com/admire/'+userid+'?format=xml&from='+postsfrom.admires,
    		    cache:false,
    		    dataType:"xml",
    		    success:function(xml){
    	      ...
    	      $(xml).find('admires').each(function(){
    	    	  $(this).find('admire').each(function(){
          	        ...
          	        top.style.width='700px';
          	          ...
          	          var b=document.createElement('div');
          	             b.className='comment-tab';
          	             b.innerHTML='<div style="width:670px; float:left; "><a id="admire-del'+$(this).find('id').text().trim()+'" class="remove"data="'+$(this).find('id').text().trim()+'" title="delete admire" style="float:right; font-size:12px; postion:absolute;  cursor:pointer; margin-right:-5px;">x</a><label style="font-size:24px; padding:5px; font-weight:bold"><a href="http://www.freniz.com/'+$(this).find('suserid').text()+'">'+$(this).find('susername').text()+'</a></label><br/><span style="font-size:14px; margin-left:10px; font-weight:bold; color:#ccc;"><div class="timeago" style="float:left; margin-top:-5px; margin-left:5px;" title="'+$(this).find('date').text()+'">'+$(this).find('date').text()+'</div></span></div><div style="width:540px; padding:5px; text-align:justify; margin-left:10px; float:left;">'+$(this).find('message').text()+'</div><div class="remove-items'+$(this).find('id').text()+'"id="remove-items"style="width:90px; display:none; background-color:#eee;float:left; "><ul><li class="delete-'+$(this).find('id').text()+'"><a href="javascript:void(0)">delete</a></li><li><a href="javascript:void(0)">settings</a></li><li><a href="javascript:void(0)">Report</a></li></ul></div>';
      ...
      function createadmirestatus(userid)
 	  {
 	      ...
 	      $.ajax({
      		url:'http://www.freniz.com/addadmiration',
      		cache:false,
      		data:parameters,
      		type:'post',
      		dataType:"json",
 	  ...
 	  function admirestatus()
 	  {
 	   ...
 	   function deleteadmire(admireid){
     	  $.ajax({
   		    url:'http://www.freniz.com/deleteadmire?admireid='+admireid,
   		    cache:false,
   		    dataType:"json",
   		    success:function(json){
		 	...
		 	{
		 		 $.ajax({
		    		    url:'http://www.freniz.com/'+userid+'/albums?format=xml',
		    		    cache:false,
		    		    dataType:"xml",
		    		    success:function(xml){
		    		    	 getalbum(xml);
		 	           ...
		 	           $(xml).find('albums').each(function(){
			    	    	  $(this).find('album').each(function(){
		 	                    var album=document.createElement('div');
		 	                      album.innerHTML='<a style="font-weight:bold; color:#444" href="http://www.freniz.com/getimages?albumid='+$(this).find('id').text()+'"><div style="width: 320px; margin: 20px; border: solid 1px; float: left"><div style="width: 100px; height: 100px; margin: 10px; border: solid 1px; float: left"><img src="http://images.freniz.com/75/75_'+$(this).find('coverurl').text()+'" height="100" width="100"/></div><div style="width: 190px; margin-top: 20px; height: 20px; border: solid 1px; float: left">'+$(this).find('name').text()+'</div><div style="width: 190px; margin-top: 5px; height: 20px; border: solid 1px; float: left" class="timeago" title="'+$(this).find('date').text()+'">'+$(this).find('date').text()+'</div></div></a>';
		 	                   $(e).append(album);
		 	...
		 	{
		 		 $.ajax({
		    		    url:'http://www.freniz.com/videos/'+userid+'?format=xml',
		    		    cache:false,
		    		    dataType:"xml",
		    		    success:function(xml){
		    		    	 getvideo(xml);
		 	...
		 	{
		 			 $.ajax({
		    		    url:'http://www.freniz.com/getvideos?videoid='+videoid+'&format=xml',
		    		    cache:false,
		    		    dataType:"xml",
		    		    success:function(xml){
		 	...
		 	function getvideosingle(xml,videoid){
		 		 var top=document.getElementById("video-div");
		            ...
		            f.innerHTML='<div id="comment-feild"style="width: 560px;height: 25px;padding:5px; background-color:#ccc; float: left;"><input id="text-comment" onkeydown="docommentvideo('+$(this).find('videoid').text()+',event)" type="text"placeholder="write to comment"style="width: 560px; height: 25px;"/></div>';
		            $(b).append(f);
		            $(main).append(b);
			...
			function loadvideocomments(videoid){
				//alert("http://www.freniz.com/scribbles/getcomments?scribbleid="+scribbleid+"&from="+comments.scribbles[scribbleid]['commentsdisplayed']);
				$.ajax({
					url:"http://www.freniz.com/videos/getcomments/videoid/"+videoid+"?from="+comments.videos[videoid]['commentsdisplayed'],
					dataType:"xml",
					cache:false,
					success:function(xml){
		 			...
		 			keynum = e.which;
		 		        ...
		 		        if(keynum==13 || !e){
		 		        	...
		 		        	if(text!=''){
		 	                 ...
		 	                 $.ajax({
		 	              	  type: 'POST',
		 	              	  url: 'http://www.freniz.com/image/comment/imageid/'+imageid,
		 	              	  data: parameters,
		 	              	  success: function(json){
	            ...
	            {	
		 		...
		 		if(window.event) // IE8 and earlier
		 			...
		 			keynum = e.which;
		 		        ...
		 		        if(keynum==13 || !e){
		 		        	var text=document.getElementById('text-comment').value;
		 		        	if(text!=''){
		 		        	  var parameters="videoid="+videoid+"&text="+text;
		 	                 $.ajax({
		 	              	  type: 'POST',
		 	              	  url: 'http://www.freniz.com/docommentvideo',
		 	              	  data: parameters,
		 	...
		 	function votevideo(videoid,element){
	 			 $.ajax({
	    		    url:'http://www.freniz.com/votevideo?videoid='+videoid,
	    		    cache:false,
	    		    dataType:"json",
	    		    success:function(json){
	    		    	var span=document.createElement('div');
		 	...
		 	function unvotevideo(videoid,element){
	 			 ...
	 			 $.ajax({
	    		    url:'http://www.freniz.com/unvotevideo?videoid='+videoid,
	    		    cache:false,
	    		    dataType:"json",
	    		    success:function(json){
	    		    	if(json.status=='success'){
		 	...
		 	function getimages(albumid){
		 		 $.ajax({
		    		    url:'http://www.freniz.com/getimages?albumid='+albumid+'&format=xml&from='+postsfrom.images,
		    		    cache:false,
		    		    dataType:"xml",
		    		    success:function(xml){
		 	...
		 	function getimage(xml,albumid){
				$('#loadingimages').remove();
		 		 var main=document.getElementById("image-div");
		            ...
		            e.innerHTML='<a href="http://www.freniz.com/image/'+$(this).find('albumid').text()+'#'+$(this).find('id').text()+'"><div style=" margin: 10px; border: solid 1px; float: left"><div style=" margin: 5px; border: solid 1px; float: left"><img src="http://images.freniz.com/200/200_'+$(this).find('url').text()+'"/></div></div></a>';
		            $(main).append(e);
		    	    	  });
		    	    	  if($(this).find('loadmore').text()=='yes'){
							var loading=document.createElement('div');
		 	...
		 	function deleteimagecomment(commentid){
		 		 $.ajax({
		    		    url:'http://www.freniz.com/image/deleteimagecomment/commentid/'+commentid,
		    		    cache:false,
		    		    dataType:"json",
		    		    success:function(json){
		 	...
		 	function displayimage(imageid)
		 	{
				...
				$.ajax({
					url:"http://www.freniz.com/image/getpinpeople/imageid/"+imageid,
					dataType:"xml",
					cache:false,
					success:function(xml){
						var a=document.createElement('div');
		 	...
		 	function getImageComments(imageid){
				$.ajax({
				url:"http://www.freniz.com/image/getcomments/imageid/"+imageid+"?from="+comments.images[imageid]['commentsdisplayed'],
				dataType:"xml",
				cache:false,
				success:function(xml){
					clearTimeout(loadCommentsTimeout);
		 	...
		 	function getscribbles(userid){
		 		 $.ajax({
		    		    url:'http://www.freniz.com/scribbles?userid='+userid+'&format=xml&from='+postsfrom.scribbles,
		    		    cache:false,
		    		    dataType:"xml",
		    		    success:function(xml){
		            	 ...
		            	 pic.innerHTML='<img id="user-pic" src="http://images.freniz.com/75/75_'+$(this).find('suserpic').text()+'"/>';
		            	  ...
		            	  var a=document.createElement('div');
		                   ...
		                   a.style.padding='5px';
		                 	...
		                 	c.style.width='500px';
		                 	c.style.float='left';
		                 	c.innerHTML='<div style="width:500px; float: left; font-size: 8px; margin-top: 2px; height: 45px; "><div style="width: 10px; height: 10px; position: absolute; margin-left: 480px;margin-top: 5px;  float: right"><a id="remove-'+$(this).find('id').text()+'" class="close"data="'+$(this).find('id').text()+'"href="javascript:void(0)">X</a></div><div style="width:420px; float: left; font-size: 24px; font-weight: bold; height: 25px; "><a class="hover" data="'+$(this).find('suserid').text()+'" href="http://www.freniz.com/'+$(this).find('suserid').text()+'">'+$(this).find('susername').text()+'</a><span style="margin-left:5px;font-size:14px; color:#ccc">'+$(this).find('pt').text()+'</span></div><div style="width:400px; float: left; font-size: 12px; font-weight: bold; height: 15px; margin-left:10px; color: #aaa; "><div class="timeago" style="float:left; margin-top:2px;" title="'+$(this).find('date').text()+'">'+$(this).find('date').text()+'</div></div></div><div style="width:460px; margin-left: 20px; float: left; margin-top: 5px; padding:5px; max-height: 200px; ">'+$(this).find('status').text()+'</div>';
		                  var b=document.createElement('div');
		                 b.style.width='480px';
		                 b.style.padding='5px';
		                 b.style.float='left';
		                 if($(this).find('votecontains').text().trim()=='no'){
		                	 b.innerHTML='<a onclick="votescribbles('+$(this).find('id').text()+',this)" class="vote-bar"style="float: right">wink( '+$(this).find('vote_count').text()+' )</a>';
		                  }else{
		                	  b.innerHTML='<a onclick="unvotescribbles('+$(this).find('id').text()+',this)" class="vote-bar"style="float: right">unwink( '+$(this).find('vote_count').text()+' )</a>';
		                  }
		                 $(c).append(b);
		 	 ...
		 	 function loadscribblecomments(scribbleid){
					//alert("http://www.freniz.com/scribbles/getcomments?scribbleid="+scribbleid+"&from="+comments.scribbles[scribbleid]['commentsdisplayed']);
					$.ajax({
						url:"http://www.freniz.com/scribbles/getcomments?scribbleid="+scribbleid+"&from="+comments.scribbles[scribbleid]['commentsdisplayed'],
						dataType:"xml",
						cache:false,
						success:function(xml){
								$(xml).find('comments').each(function(){
		 	...
		 	function loadcomments(){
				var url="http://www.freniz.com/comment/getcomments?maxcomment="+comment_max;
				var statures=Array();
							...
							$(this).find('comment').each(function(){
								switch($(this).find('type').text()){
									...
									case 'image':
										var d=document.createElement('div');
										d.id='comment-box'+$(this).find('id').text();
										d.innerHTML='<div style="width: 500px; margin-top:5px; padding-bottom:5px; float: left; border-bottom: solid 1px #fff"><div style="width: 32px; height: 32px; float: left;"><img height="32" width="32" src="http://images.freniz.com/32/32_'+$(this).find('user_image').text()+'"/></div><div style="width: 460px; max-height: 100px; margin-left: 5px; margin-top: 5px; float: left; "><label style="padding-right:2px; font-weight:bold;">'+$(this).find('username').text()+':</label>'+$(this).find('comment-comment').text()+'</div><a href="javascript:void(0)" onclick="deleteimagecomment('+$(this).find('id').text()+')" style="float:right; margin-top:-20px;">x</a><div class="timeago" style="height:10px; margin-left: 5px; font-size:10px; float: left"title="'+$(this).find('date').text()+'">'+$(this).find('date').text()+'</div></div>';
		 		        ...
		 		        if(keynum==13 || !e){
		 		        	...
		 		        	if(text!=''){
		 		        	  var parameters="postid="+postid+"&text="+text;
		 		        	   ...
		 		        	   $.ajax({
		 	              	  type: 'POST',
		 	              	  url: 'http://www.freniz.com/docommentscribbles',
		 	              	  data: parameters,
		 	              	  success: function(json){alert(json.status);},
		 	              	  dataType: "json"
		 	              	});
		 	 ...
		 	 function deletescribbles(postid){
		 		 $.ajax({
	    		    url:'http://www.freniz.com/deletescribbles?postid='+postid,
	    		    cache:false,
	    		    dataType:"json",
	    		    success:function(json){
	    		    	$(".user-pic"+postid).remove();
	    	   		 	$('#remove-items').css({'position':'none','display':'none'});
	    	   		 $('.stature-stream'+postid).css({'display':'none'});
		 	 ...
		 	 function deletescribblescomment(commentid){
		 		 $.ajax({
		    		    url:'http://www.freniz.com/deletescribblescomment?commentid='+commentid,
		    		    cache:false,
		    		    dataType:"json",
		    		    success:function(json){
			 ...
			 function deletevideocomment(commentid){
		 		 $.ajax({
		    		    url:'http://www.freniz.com/deletevideocomment?commentid='+commentid,
		    		    cache:false,
		    		    dataType:"json",
		    		    success:function(json){
			 ...
			 function deletevideo(videoid){
		 		 $.ajax({
		    		    url:'http://www.freniz.com/deletevideo?videoid='+videoid,
		    		    cache:false,
		    		    dataType:"json",
		    		    success:function(json){
		    		    	window.location.href="http://www.freniz.com/videos/"+json.userid;
		    		    }
		 		    } );
		 	...
		 	function votescribbles(postid,element){
	 			 ...
	 			 $.ajax({
	    		    url:'http://www.freniz.com/votescribbles?postid='+postid,
	    		    cache:false,
	    		    dataType:"json",
	    		    success:function(json){
	    		    	 if(json.status=='success'){
		 	...
		 	function unvotescribbles(postid,element){
		 		 ...
		 		 $.ajax({
	    		    url:'http://www.freniz.com/unvotescribbles?postid='+postid,
	    		    cache:false,
	    		    dataType:"json",
	    		    success:function(json){
	    		    	if(json.status=='success'){
		 	...
		 	function postvideos(userid){
		 		...
		 		var embeddcode=document.getElementById("video-embeddcode").value;
		        	  ...
		        	  $.ajax({
	              	  type: 'POST',
	              	  url: 'http://www.freniz.com/addvideo',
	              	  data: parameters,
	              	  success: function(json){
		 	...
		 	function postscribbles(userid){
		 		...
		 		var cpt=$('#comment-pt').val();
		        	  ...
		        	  var parameters="userid="+userid+"&text="+text+"&pt="+pt+'&cpt='+cpt;
		        	    $.ajax({
	              	  type: 'POST',
	              	  url: 'http://www.freniz.com/addscribbles',
	              	  data: parameters,
	              	  success: function(json){alert(json.status);},
	              	  dataType: "json"
	              	});
		 	...
		 	function getstatures(userid){
		 		 $.ajax({
		    		    url:'http://www.freniz.com/getuserstatures?userid='+userid+'&format=xml&from='+postsfrom.statures,
		    		    cache:false,
		    		    dataType:"xml",
		    		    success:function(xml){
		    		  ...
		    		  $(this).find('stature').each(function(){
		    			  var statureid=$(this).find('statureid').text();
		                   ...
		                   a.style.padding='5px';
		                 	...
		                 	c.style.width='500px';
		                 	c.style.float='left';
		                 	c.innerHTML='<div style="width:500px; float: left; font-size: 8px; margin-top: 2px; height: 45px; "><div style="width: 10px; height: 10px; position: absolute; margin-left: 480px;margin-top: 5px;  float: right"><a id="remove-'+$(this).find('statureid').text()+'" class="close"data="'+$(this).find('statureid').text()+'"href="javascript:void(0)">X</a></div><div style="width:420px; float: left; font-size: 24px; font-weight: bold; height: 25px; ">'+$(this).find('username').text()+'</div><div style="width:400px; float: left; font-size: 12px; font-weight: bold; height: 15px; margin-left:10px; color: #aaa; "><div class="timeago" style="float:left; margin-top:2px;" title="'+$(this).find('date_stature').text()+'">'+$(this).find('date_stature').text()+'</div></div></div><div style="width:460px; margin-left: 20px; float: left; margin-top: 5px; padding:5px; max-height: 200px; ">'+$(this).find('message').text()+'</div>';
			 ...
			 function loadstaturecomments(statureid){
					$.ajax({
						url:"http://www.freniz.com/statures/getcomments?statureid="+statureid+"&from="+comments.statures[statureid]['commentsdisplayed'],
						dataType:"xml",
						cache:false,
						success:function(xml){
								$(xml).find('comments').each(function(){
		 	...
		 	function addstature(){
		 		...
		 		var cpt=$('#comment-pt').val();
		        	  ...
		        	  var parameters="text="+text+"&visi="+visi+"&cpt="+cpt;
		        	    $.ajax({
	              	  type: 'POST',
	              	  url: 'http://www.freniz.com/addstatures',
	              	  data: parameters,
	              	  success: function(json){
	              		 var main=document.getElementById('statures'); 
	              		   var a=document.createElement('div');
		                   ...
		                   a.style.padding='5px';
		                 	...
		                 	c.style.width='500px';
		                 	c.style.float='left';
		                 	c.innerHTML='<div style="width:500px; float: left; font-size: 8px; margin-top: 2px; height: 45px; "><div style="width: 10px; height: 10px; position: absolute; margin-left: 480px;margin-top: 5px;  float: right"><a id="remove-'+json.id+'" class="close"data="'+json.id+'"href="javascript:void(0)">X</a></div><div style="width:420px; float: left; font-size: 24px; font-weight: bold; height: 25px; ">'+json.username+'</div><div style="width:400px; float: left; font-size: 12px; font-weight: bold; height: 15px; margin-left:10px; color: #aaa; "><div class="timeago" style="float:left; margin-top:2px;" title="'+json.date+'">'+json.date+'</div></div></div><div style="width:460px; margin-left: 20px; float: left; margin-top: 5px; padding:5px; max-height: 200px; ">'+json.stature+'</div>';
		 	 ...
		 	 function deletestature(statureid){
		 		 $.ajax({
		    		    url:'http://www.freniz.com/deletestature?statureid='+statureid,
		    		    cache:false,
		    		    dataType:"json",
		    		    success:function(json){
		 		        ...
		 		        if(keynum==13 || !e){
		 		        	...
		 		        	if(text!=''){
		 		        	  var parameters="statureid="+statureid+"&text="+text;
		 		        	   $.ajax({
		 	              	  type: 'POST',
		 	              	  url: 'http://www.freniz.com/dostaturecomment',
		 	              	  data: parameters,
		 	              	  success: function(json){
		 	 ...
		 	 function deletestaturecomment(commentid){
		 			 $.ajax({
		    		    url:'http://www.freniz.com/deletestaturecomment?commentid='+commentid,
		    		    cache:false,
		    		    dataType:"json",
		    		    success:function(json){
		    		    	$("#stature-comment-box"+commentid).remove();
		 	 ...
		 	 function votestature(statureid,element){
		 		 $.ajax({
		    		    url:'http://www.freniz.com/votestature?statureid='+statureid,
		    		    cache:false,
		    		    dataType:"json",
		    		    success:function(json){
		    		    	 if(json.status=='success'){
		 	 ...
		 	 function unvotestature(statureid,element){
		 		 $.ajax({
		    		    url:'http://www.freniz.com/unvotestature?statureid='+statureid,
		    		    cache:false,
		    		    dataType:"json",
		    		    success:function(json){
		    		    	if(json.status=='success'){
		 	 ...
		 	 function voteadmire(admireid,element){
		 		 $.ajax({
		    		    url:'http://www.freniz.com/voteadmire?admireid='+admireid,
		    		    cache:false,
		    		    dataType:"json",
		    		    success:function(json){
	    		    	if(json.status=='success'){
		 	 ...
		 	 function unvoteadmire(admireid,element){
		 		 $.ajax({
		    		    url:'http://www.freniz.com/unvoteadmire?admireid='+admireid,
		    		    cache:false,
		    		    dataType:"json",
		    		    success:function(json){
		    		    	if(json.status=='success'){
		 	...
		 	function voteblog(blogid,element){
		 		 $.ajax({
		    		    url:'http://www.freniz.com/voteblog?blogid='+blogid,
		    		    cache:false,
		    		    dataType:"json",
		    		    success:function(json){
		    		    	 if(json.status=='success'){
		 	 ...
		 	 function unvoteblog(blogid,element){
		 		 $.ajax({
		    		    url:'http://www.freniz.com/unvoteblog?blogid='+blogid,
		    		    cache:false,
		    		    dataType:"json",
		    		    success:function(json){
		    		    	if(json.status=='success'){
		 	  ...
		 	  {
				   $.ajax({
		    		    url:'http://www.freniz.com/setprofilepicture?imageid='+imageid+'&x='+x+'&y='+y+'&width='+width+'&height='+height+'&deletesrc='+deletesrc,
		    		    cache:false,
		    		    dataType:"json",
		    		    success:function(json){
            ...
            initcrop(2/1);
		 	 ...
		 	 {
		 		 $.ajax({
		    		    url:'http://www.freniz.com/setsecondarypicture?imageid='+imageid+'&x='+x+'&y='+y+'&width='+width+'&height='+height+'&deletesrc='+deletesrc+'&secpicno='+secpicno,
		    		    cache:false,
		    		    dataType:"json",
		    		    success:function(json){
		    		    	$('.crop-button').css('display','block');
		 	  ...
		 	  function secpicposition(imageid,top)
			 	 {
			 		 $.ajax({
			    		    url:'http://www.freniz.com/setsecondarypicture?imageid='+imageid+'&secpicno=1&top='+top,
			    		    cache:false,
			    		    dataType:"json",
			    		    success:function(json){
			    		    	 alert(json.status);
		 		...
		 		function questions(tags)
			 	{
		 			document.getElementById("load").style.display='block';
			 		 $.ajax({
			    		    url:'http://www.freniz.com/forum/search?type=tags&order=popular&key='+tags,
			    		    cache:false,
			    		    dataType:"xml",
			    		    success:function(xml){
			    		    	 question(xml);
			 ...
			 //		var key=document.getElementById('find-ques').value;
			 		//http://www.freniz.com/forum/search?type=tags&order=popular&key=php
			 		if(key!=''){
			 			 $.ajax({
				    		    url:'http://www.freniz.com/forum/search?type=question&order=popular&key='+key,
				    		    cache:false,
				    		    dataType:"xml",
				    		    success:function(xml){
				    		    	 question(xml);
			 	...
			 	function topics(key){
			 		document.getElementById("load").style.display='block';
			 		 $.ajax({
			    		    url:'http://www.freniz.com/forum/search?type=tags&order=popular&key='+key,
			    		    cache:false,
			    		    dataType:"xml",
			    		    success:function(xml){
			    		    	question(xml);
			 	...
			 	function removefriends(userid){
			 		 $.ajax({
			    		    url:'http://www.freniz.com/profile/removefriends?userid='+userid,
			    		    cache:false,
			    		    dataType:"json",
			    		    success:function(json){
			    		    	alert(json.status);
			 	...
			 	function addfriends(userid){
			 		 $.ajax({
			    		    url:'http://www.freniz.com/profile/addfriends?userid='+userid,
			    		    cache:false,
			    		    dataType:"json",
			    		    success:function(json){
			    		    	var span=document.createElement('div');
			 	...
			 	function acceptfriends(userid){
			 		 $.ajax({
			    		    url:'http://www.freniz.com/profile/acceptfriends?userid='+userid,
			    		    cache:false,
			    		    dataType:"json",
			    		    success:function(json){
			    		    	alert(json.status);
			 	...
			 	function denyfriends(userid){
			 		 $.ajax({
			    		    url:'http://www.freniz.com/profile/denyfriends?userid='+userid,
			    		    cache:false,
			    		    dataType:"json",
			    		    success:function(json){
			    		    	alert(json.status);
			 	...
			 	function uservote(userid,element){
			 		alert('http://www.freniz.com/profile/uservote?userid='+userid);
			 		 $.ajax({
			    		    url:'http://www.freniz.com/profile/uservote?userid='+userid,
			    		    cache:false,
			    		    dataType:"json",
			    		    success:function(json){
			    		    	var span=document.createElement('div');
			 	...
			 	function userunvote(userid,element){
			 		 $.ajax({
			    		    url:'http://www.freniz.com/profile/userunvote?userid='+userid,
			    		    cache:false,
			    		    dataType:"json",
			    		    success:function(json){
			    		    	var span=document.createElement('div');
			 	...
			 	{
			 		 $.ajax({
			    		    url:'http://www.freniz.com/answers/'+questionid,
			    		    cache:false,
			    		    dataType:"xml",
			    		    success:function(xml){
			    		    	 answer(xml);
		 		        ...
		 		        if(keynum==13 || !e){
		 		        	...
		 		        	if(text!=''){
		 		        	  var parameters="questionid="+questionid+"&answer="+text+"&date="+date;
		 		        	   $.ajax({
		 	              	  type: 'POST',
		 	              	  url: 'http://www.freniz.com/ansquestion',
		 	              	  data: parameters,
		 	              	  success: function(json){alert(json.status);},
		 	              	  dataType: "json"
		 	              	});
	            ...
	            {	
		 		        ...
		 		        if(keynum==13 || !e){
		 		        	...
		 		        	if(text!=''){
		 		        	  var parameters="answerid="+answerid+"&comment="+text;
		 		        	    $.ajax({
		 	              	  type: 'POST',
		 	              	  url: 'http://www.freniz.com/commentanswer',
		 	              	  data: parameters,
		 	              	  success: function(json){alert(json.status);},
		 	              	  dataType: "json"
		 	              	});
				 ...
				 function votequestion(questionid){
					 $.ajax({
			    		    url:'http://www.freniz.com/votequestion?questionid='+questionid,
			    		    cache:false,
			    		    dataType:"json",
			    		    success:function(json){
			    		    	 alert(json.status);
				 ...
				 function voteanswer(answerid){
					 $.ajax({
			    		    url:'http://www.freniz.com/voteanswer?answerid='+answerid,
			    		    cache:false,
			    		    dataType:"json",
			    		    success:function(json){
			    		    	 alert(json.status);
				 ...
				 function findtags(element,event)
					 ...
					 var key=element.value;
				 	 $.ajax({
				    		    url:'http://www.freniz.com/forum/gettags?key='+key,
				    		    cache:false,
				    		    dataType:"xml",
				    		    success:function(xml){
				 ...
				 function searchname()
					 ...
					 $.ajax({
				    		    url:"http://www.freniz.com/search?name="+key.trim()+"&type="+type,
				    		    cache:false,
				    		    dataType:"xml",
				    		    success:function(xml){
				 ...
				 function findskills(element,event)
					 ...
					 var key=element.value;
				 	 $.ajax({
				    		    url:'http://www.freniz.com/settings/getskills?key='+key,
				    		    cache:false,
				    		    dataType:"xml",
				    		    success:function(xml){
				    		    	 findtag(xml,element,event);
				  ...
				  function askquestion()
			       {
			         ...
			         $('#tag-div').children('div').each(function(){
			          ...
			          if(text=='' || tags==''){alert("please fill the information");}else{
			            	...
			            	$.ajax({
			            		url:'http://www.freniz.com/askquestion',
			            		cache:false,
			            		data:parameters,
			            		type:'post',
			            		dataType:"json",
				 ...
				 function changepage(){
					   var key=document.getElementById('welcome-search-element').value;
					    ...
					    if(key.trim()=='' && type!='skills'){
					    		window.location.href='http://www.freniz.com/frenizsearch?s=';
					    }else{
					    	  var category=document.getElementById('category').value;
					           ...
					           var placetype=document.getElementById('select-field-place').value;
					           	var query;
					           	query ='http://www.freniz.com/frenizsearch?s='+key+'&type='+type;
					        	if(places!='' && type=='user')
				 ...
				 function welcomesearch(type,key,category,subcategory,skills,ccity,htown){
				    ...
				    if(type=='user' && ccity=='' && htown==''){
				              ...
				              url="http://www.freniz.com/search?key="+key.trim()+"&type="+type;    
				       
				      }else if(type=='user' && ccity!=''){
							url="http://www.freniz.com/search?key="+key.trim()+"&ccity="+ccity+"&type="+type;
				      }
				      else if(type=='user' && htown!=''){
							url="http://www.freniz.com/search?key="+key.trim()+"&htown="+htown+"&type="+type;
				      }
				    
				      else if(type=='page'){
                            url="http://www.freniz.com/search?key="+key.trim()+"&category="+escape(category)+"&subcategory="+subcategory+"&type="+type;
				        ...
				        if(type=='skills'){
				        	 url="http://www.freniz.com/search?key="+key.trim()+"&skills="+skills+"&type=user";
					      }
				        $.ajax({
		 	              	  type: 'get',
		 	              	  url: url,
				 ...
				 function checkusername()
				 {
					 ...
					 if(e.value.indexOf(' ')==-1)
				         {
				    	     $.ajax({
				            	 url:'http://www.freniz.com/createusr/checkuser?username='+e.value,
				            	 dataType:"json",
				            	 cache:false,
				            	 success:function(json){
				            		 if(json.status=='true')
				 ...
				 function checkemail()
				{
					...
					if(value.indexOf(' ')==-1 && value.indexOf('@')!=-1 && value.indexOf('.')!=-1 && value.indexOf('@')!=0 && value.indexOf('@')!=value.length-1 && value.indexOf('.')!=0 && value.indexOf('.')!=value.length-1)
						{
							$.ajax({
								url:"http://www.freniz.com/createusr/checkmail?email="+value,
								dataType:"json",
								cache:false,
								success:function(json){
									if(json.status=='true')
				     ...
				     if(un.style.backgroundColor=='rgb(169, 245, 169)' && pass.style.backgroundColor=='rgb(169, 245, 169)' && cpass.style.backgroundColor=='rgb(169, 245, 169)' && email.style.backgroundColor=='rgb(169, 245, 169)' && fname.value!='' && lname.value!='' && sex.value!=0 && bdd.value!=-1 && bdm.value!=-1 && bdy.value!=-1)
				         {
				             ...
				             $.ajax({
				    	 		url:'http://www.freniz.com/index/signup',
				    	 		data:parameters,
				    	 		dataType:"json",
				    	 		cache:false,
				    	 		type:"post",
				    	 		success:function(json){
									if(json.status=='true'){
						             login_auth(json.un,json.pass,'http://www.freniz.com/index/personalinfo?log_aug=true');
						             }
						             else
						             alert(json.message);
				    	 			}
				     ...
				     if(pass.style.backgroundColor=='rgb(169, 245, 169)' && cpass.style.backgroundColor=='rgb(169, 245, 169)' && email.style.backgroundColor=='rgb(169, 245, 169)' && bdd.value!=-1 && bdm.value!=-1 && bdy.value!=-1)
				         {
				             ...
				             $.ajax({
				    	 		url:'http://www.freniz.com/leaf/createleafaccount',
				    	 		data:parameters,
				    	 		dataType:"json",
				    	 		cache:false,
				    	 		type:"post",
			...
			function personalinfo()            
            {
             ...
             $.ajax({
              url:'http://www.freniz.com/updatepersonalinfo',
              data:parameters,
              cache:false,
              dataType:"json",
              type:"post",
            ...
            function createpage(pagename,category,subcategory,type,fav,option)
			{
				...
				if(!type)
					type='default'
					var parameters="type="+type+"&pagename="+pagename+"&category="+category+"&subcategory="+subcategory;
					if(fav)
					parameters+="&fav="+fav;
					$('#loading').css('display','block');
					$.ajax({
						url:'http://www.freniz.com/leaf/create',
						data:parameters,
						dataType:'json',
						cache:false,
						success:function(json){
		 ...
		 function addfavorites(pageids,category,type)
				 {
					 ...
					 if(type=='school' || type=='college')
						url='http://www.freniz.com/updatetofavorites?pageids='+pageids+'&category='+category+'&type='+type+"&from=2007&end=2011";
					 else
						url='http://www.freniz.com/updatetofavorites?pageids='+pageids+'&category='+category+'&type='+type;
					 if(pageids!=0){
				    	 $.ajax({
		 	              	  type: 'get',
		 	              	  url: url,
				...
				setTimeout(function(){$('#favr-info-message').remove();},10000);  
				 ...
				 function getfavsug(){
					...
					var url='http://www.freniz.com/search?type=page&key='+key;
					if(category!='other')
					  ...
					  $(xml).find('pages').each(function(){
			                 $(this).find('page').each(function(){
			                	...
			                	if(!document.getElementById('sug-'+$(this).find('pageid').text())){
			                  	 var a=document.createElement('div');
								 a.innerHTML='<div id="sug-'+$(this).find('pageid').text()+'" class="show"style=" width:100px; overflow:hidden; margin-top:5px; margin-left:5px;float:left; position:relative; height:100px; border:solid 1px"><a id="add-fav'+$(this).find('pageid').text()+'" class="fav-show"style="margin:20px; font-size:18px;text-decoration:none; font-weight:bold"onClick="addtofavr(\''+$(this).find('pageid').text()+'\',\''+category+'\')"href="javascript:void(0)">Add</a><a id="remo-fav'+$(this).find('pageid').text()+'" class="fav-hidden"style="margin:20px; font-size:18px;text-decoration:none; font-weight:bold"onClick="removefromfavr(\''+$(this).find('pageid').text()+'\',\''+category+'\')"href="javascript:0">remove</a><div style=" width:100px; position:absolute; bottom:0; opacity:0.6; color:#000;  filter:alpha(opacity=60);"><a href="http://www.freniz.com/'+$(this).find('pageid').text()+'">'+$(this).find('pagename').text()+'</a></div></div>';
								 b.appendChild(a); 
							 }
			                 });
					  });
           ...
           function addtofavr(id,category){
			...
			function updatemood(){
				...
				var description=$('#mood-description').val();
		 		 $.ajax({
		    		    url:'http://www.freniz.com/settings/updatemood?mood='+mood+'&description='+description,
		    		    cache:false,
		    		    dataType:"json",
		    		    type:'post',
		    		    success:function(json){
		    		    	$('#mood-smile').css('display','none');
		    		    	$('#top-smiley').attr('src','http://www.freniz.com/images/mood/32/'+json.mood);
		    		    if(id==json.id){
			...
			function updatehometown(){
				...
				$.ajax({
					url:"http://www.freniz.com/settings/updatecity?type=hometown&city="+city,
					dataType:'json',
					cache:false,
					success:function(json){
			...
			function updatelivingin(){
				...
				$.ajax({
					url:"http://www.freniz.com/settings/updatecity?type=currentcity&city="+city,
					dataType:'json',
					cache:false,
					success:function(json){
			...
			function changemood(mood){
				$('#mood-image').attr('src','http://www.freniz.com/images/mood/32/'+mood);
			...
			function pinpeople(imageid){
				//alert("http://www.freniz.com/image/addpin/imageid/"+imageid+"?userids="+pinpeoples);
				var pinnedpeople='';
				...
				$.ajax({
					url:"http://www.freniz.com/image/addpin/imageid/"+imageid+"?userids="+pinnedpeople,
					dataType:"json",
					cache:false,
					success:function(json){
						alert('success');
			...
			function adddescription(){
				...
				if(text!=''){
		        	  var parameters="text="+text;
		        	   $.ajax({
	              	  type: 'POST',
	              	  url: 'http://www.freniz.com/image/adddescription/imageid/'+imageid,
	              	  data: parameters,
	              	  success: function(json){
			...
			function pinme(imageid){
				$.ajax({
					url:"http://www.freniz.com/image/pinmereq/imageid/"+imageid+"/",
					dataType:"json",
					cache:false,
					success:function(json){
								alert('success');
			...
			function unpin(imageid,userid){
				$.ajax({
					url:"http://www.freniz.com/image/unpin/imageid/"+imageid+"/userid/"+userid+"/",
					dataType:"json",
					cache:false,
					success:function(json){
						alert('success');
			...
			function voteimage(imageid,element){
				 $.ajax({
		    		    url:'http://www.freniz.com/image/voteimage/imageid/'+imageid,
		    		    cache:false,
		    		    dataType:"json",
		    		    success:function(json){
		    		    	 if(json.status=='success'){
			...
			function deleteimage(imageid){
				 $.ajax({
		    		    url:'http://www.freniz.com/image/deleteimages/imageid/'+imageid,
		    		    cache:false,
		    		    dataType:"json",
		    		    success:function(json){
		    		    	 location.reload();
		 	 ...
		 	 function unvoteimage(imageid,element){
		 		 ...
		 		 $.ajax({
		    		    url:'http://www.freniz.com/image/unvoteimage/imageid/'+imageid,
		    		    cache:false,
		    		    dataType:"json",
		    		    success:function(json){
		    		    	if(json.status=='success'){
			...
			function getmessages()
			{
				...
				$.ajax({
					url:'http://www.freniz.com/messages?format=xml',
					cache:false,
					dataType:"xml",
				 ...
				 $(xml).find('messages').each(function(){
			         $(this).find('message').each(function(){
			             ...
			             a.id="message-"+$(this).find('suserid').text();
							...
							top.cssFloat="left";
							top.href='http://www.freniz.com/messages/'+$(this).find('suserid').text();
							  top.innerHTML='<div style="width:500px; border-bottom:solid 1px; padding:10px;float:left; height: 50px; "><div style="width:50px; float:left; height: 50px;"><img width="50" height="50" src="http://images.freniz.com/50/50_'+$(this).find('propic').text()+'"/></div><div style="width:435px; margin-left:5px; padding:5px; float:left; height: 40px; "><label style="font-size:19px; font-weight: bold"><a href="http://www.freniz.com/'+$(this).find('suserid').text()+'"style="text-decoration:none; float:left;padding-right:5px; font-weight: bold; color:#000; font-size:20px;">'+$(this).find('from').text()+'</a></label><div style="float:left;color:#ccc;margin-top:3px;" class="timeago" title="'+$(this).find('date').text().trim()+'">'+$(this).find('date').text()+'</div><br/><br/><div style=" width:20px; height:20px; float:left; padding:2px; margin-top:-10px; border:solid 1px"></div><span style="float:left; margin-left:3px">'+$(this).find('msg').text()+'</span</div></div>';
			...
			{
				$.ajax({
					url:"http://www.freniz.com/messages/"+userid+"?format=xml",
					cache:false,
					dataType:"xml",
			...
			function sendmessageuser(userid,e){
			 ...
			 function deleteallmessages(userid){
		    	  $.ajax({
		  		    url:'http://www.freniz.com/deletemessage?userid='+userid,
		  		    cache:false,
		  		    dataType:"json",
		  		    success:function(json){
		  		    	if(json.status=='success')
			 ...
			 function switchuser(id){
	    		  $.ajax({
			  		    url:'http://www.freniz.com/index/switch?id='+id,
			  		    cache:false,
			  		    dataType:"json",
			  		    success:function(json){
			  		    	if(json.status=='success')
			  		    		window.location.href="http://www.freniz.com/"+id;
			  		    	else alert(json.status);
			 ...
			 function addtolist(id){
	    		  $.ajax({
			  		    url:'http://www.freniz.com/leaf/addtags?tagid='+id,
			  		    cache:false,
			  		    dataType:"json",
			  		    success:function(json){
			  		    	alert(json.status);
			 ...
			 function accepttolist(id){
				var leafid= $('#search-tags').attr('data');
	    		  $.ajax({
			  		    url:'http://www.freniz.com/leaf/accepttags?tagid='+id,
			  		    cache:false,
			  		    dataType:"json",
			  		    success:function(json){
			  		    	if(json.status=='success'){
			 ...
			 function getleaftaglist(leafid)
				{
					$.ajax({
						url:"http://www.freniz.com/leaf/getleaftaglist?leafid="+leafid,
						cache:false,
						dataType:"xml",
			 ...
			 function taglist(xml){
				 ...
				 $(xml).find('pages').each(function(){
			         $(this).find('page').each(function(){
			        	...
			        	b.innerHTML='<label><a href="http://www.freniz.com/'+$(this).find('leafurl').text()+'">'+$(this).find('leafname').text()+'</a></label><br/><span><a onclick="addfavorites(\''+$(this).find('leafid').text()+'\',\''+$(this).find('category').text()+'\',\'add\')">winked( '+$(this).find('vote_count').text()+' )</a></span>';
			        	else
			        	b.innerHTML='<label><a href="http://www.freniz.com/'+$(this).find('leafurl').text()+'">'+$(this).find('leafname').text()+'</a></label><br/><span><a onclick="addfavorites(\''+$(this).find('leafid').text()+'\',\''+$(this).find('category').text()+'\',\'remove\')">wink( '+$(this).find('vote_count').text()+' )</a></span>';
			        	$(a).append(b);
			        	$(top).append(a);
			         });
				 });
			...
			//	 getrequesttags();
			 ...
			 function searchtags(key,leafid){
				 $.ajax({
						url:"http://www.freniz.com/leaf/searchtags?leafid="+leafid+"&key="+key,
						cache:false,
						dataType:"xml",
			 ...
			 function getrequesttags(){
				 $.ajax({
						url:"http://www.freniz.com/leaf/getrequesttag",
						cache:false,
						dataType:"xml",
			 ...
			 function requesttag(xml){
				 ...
				 $(xml).find('pages').each(function(){
			         ...
			         $(this).find('page').each(function(){
			        	...
			        	b.innerHTML='<label><a href="http://www.freniz.com/'+$(this).find('leafurl').text()+'">'+$(this).find('leafname').text()+'</a></label><br/><span><a onclick="accepttolist(\''+$(this).find('leafid').text()+'\');">Accept</a></span>';
			        	$(a).append(b);
			        	 $(main).append(a);
			         });
					 }
			 ...
			 function getminiprofile(id,top,left){
				 ...
				 $.ajax({
						url:"http://www.freniz.com/index/getminiprofile?id="+id,
						cache:false,
						dataType:"xml",
				 ...
				 $(xml).find('profile').each(function(){
			         $(this).find('user').each(function(){
			        	...
			        	var a1=document.createElement('div');
			        	 ...
			        	 aa.href="http://www.freniz.com/"+$(this).find('friendurl').text();
			        	...
			        	if($(this).find('ismyid').text()=='no' && $(this).find('type').text()=='user'){
			        	 var b=document.createElement('div');
			        	b.innerHTML='<span class="mes-text" style="float:right; font-size: 12px; margin-top:10px;"id="leaf-vote">send text</span><span style="float:right; font-size: 12px; margin-top:10px; margin-right:5px"onclick=""id="leaf-vote">add request</span><div id="mes-text" style="width:320px; display:none; position:absolute; margin-top:50px; margin-left:10px; float:left; padding:5px; background-color:#ccc; border:solid 1px"><div style="width:240px; padding-left:5px; margin-left:5px; float:left;  border-left:solid 1px #fff"><textarea id="messag-text" style="width:240px; outline:none; height:35px"placeholder="write a message to your friends"></textarea></div><input class="greenbutton" style="float:right; margin-top:7px" type="button" value="send" onclick="sendmsguser(\''+$(this).find('userid').text()+'\',this)"/></div>';
			        	$(a1).append(b);
			        	}
			        	$(a).append(a1);
			        	$(main).html(a);
			 ...
			 function getleafalbum(leafid){
				 $.ajax({
						url:"http://www.freniz.com/leaf/getleafalbum?leafid="+leafid,
						cache:false,
						dataType:"xml",
			 ...
			 function createalbumname(){
				 ...
				 $.ajax({
						url:"http://www.freniz.com/albums/createalbum?name="+albumname,
						cache:false,
						dataType:"json",
						success:function(json){
							$('#album-na').remove();
			 ...
			 alert(id);
				 $.ajax({
						url:"http://www.freniz.com/leaf/getwallphotos?id="+id,
						cache:false,
						dataType:"xml",
				 ...
				 var main=document.getElementById('com-container');
					...
					$(top).css({'max-height':'400px','overflow':'auto','position':'absolute','width':'320px','top':'40%','left':'40%','background-color':'#fff','border':'solid 1px','padding':'25px'});
					   top.innerHTML='Loading...';
				        	   	 $(main).append(top);
				 $.ajax({
						url:"http://www.freniz.com/profile/getfriendslist?userid="+userid,
						cache:false,
						dataType:"xml",
						success:function(xml){
							var main=document.getElementById('com-container');
			...
			function basicaccount(){
				...
				$.ajax({
					url:"http://www.freniz.com/settings/updatebasicinfo",
					data:parameters,
					dataType:"json",
					method:"post",
					cache:false,
			...
			function getcount(){
				$.ajax({
					url:"http://www.freniz.com/review/getcount",
					dataType:"json",
					cache:false,
					success:function(json){
